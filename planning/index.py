...
Run via ``just index``. Globs ``planning/deferred/*.md`` and
``planning/decisions/*.md``, reads their frontmatter, and prints a Markdown
listing to stdout — deferred (the open queue) then decisions, newest-first.
Never writes a file: the listing is a query over the files, not a committed
artifact.
...
def format_row(row: dict[str, str]) -> str:
    """Render one deferred item or decision as a Markdown list item."""
    slug = row.get("slug", "?")
    path = row.get("path", "")
    date = row.get("date", "")
...
def render(deferred: list[dict[str, str]], decisions: list[dict[str, str]]) -> str:
    """Render the full Markdown listing: deferred then decisions, newest-first."""
...
def check(root: pathlib.Path) -> list[str]:
    """Validate every deferred item and decision; return the list of violation strings."""
    violations: list[str] = []
    deferred_dir = root / "deferred"
    decisions_dir = root / "decisions"
...
def main(argv: list[str] | None = None, root: pathlib.Path | None = None) -> int:
    """Print the listing to stdout, or validate deferred items and decisions with --check."""
    ...
    if "--check" in argv:
        ...
        return 0
    sys.stdout.write(render(load_deferred(root), load_decisions(root)))
    return 0
