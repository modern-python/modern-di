...
The `scope-chain` and `circular-dependency` troubleshooting pages' Symptom
sections mention the optional trailing anchor (paraphrased, per the
no-verbatim rule); `architecture/validation.md` (cycle rendering) and the
breadcrumb coverage in `architecture/` promote the invariant.
