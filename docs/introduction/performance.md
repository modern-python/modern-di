...
> Regenerating the tables needs the isolated comparative environment and a quiet machine, so it
> is a manual step that lags a release rather than part of one. If you are running a version
> later than 3.3.0, nothing below has been re-measured against it: run `just bench-report`
> yourself (see [Reproduce it yourself](#reproduce-it-yourself)) to measure the version you
> actually have.
