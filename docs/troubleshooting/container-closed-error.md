...
**No longer raised.** As of modern-di 3.1, a container is usable immediately after construction, and
...
`ContainerClosedError` itself is kept importable for 3.x back-compat, so an `except
...
Seeing it means a reference to an already-closed container was resolved from, either directly or
through a child container whose resolve reached back into the closed container's scope, without
going back through `open()`/`with` first. Two ways to respond:
...
- **Unintentional reuse**: the warning is telling you a reference to the container is being held
  ...
  instead of fetching a fresh one). Find where that reference is coming from and fix the leak
