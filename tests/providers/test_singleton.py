...
async def test_app_singleton() -> None:
    ...
    # clear_cache=False: the instance survives the close and is returned again after the warning
    # reopens the container, without re-running the creator or finalizer.
...
def test_closed_container_warns_then_rebuilds_with_clear_cache_true() -> None:
    calls: list[str] = []
...
def test_persistent_provider_survives_close_reopen_cycle() -> None:
    ...
    # resolving while closed warns and self-heals
    with pytest.warns(ContainerClosedWarning):
        assert container.resolve(_PersistentBroker) is broker1
    # re-enter → reopen
    with container:
