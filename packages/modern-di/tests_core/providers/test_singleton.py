...
async def test_app_singleton() -> None:
    async with AsyncContainer() as app_container:
        singleton1 = await app_container.resolve_provider(app_singleton)
        singleton2 = await app_container.resolve_provider(app_singleton)
        assert singleton1 is singleton2

    with SyncContainer() as app_container:
        singleton3 = app_container.resolve_provider(app_singleton)
        singleton4 = app_container.resolve_provider(app_singleton)
        assert singleton3 is singleton4
        assert singleton3 is not singleton1

    async with AsyncContainer() as app_container:
        singleton5 = await app_container.resolve_provider(app_singleton)
        singleton6 = await app_container.resolve_provider(app_singleton)
...
def test_request_singleton() -> None:
    with SyncContainer() as app_container:
        with app_container.build_child_container(scope=Scope.REQUEST) as request_container:
            instance1 = request_container.resolve_provider(request_singleton)
            instance2 = request_container.resolve_provider(request_singleton)
            assert instance1 is instance2

        with app_container.build_child_container(scope=Scope.REQUEST) as request_container:
            instance3 = request_container.resolve_provider(request_singleton)
            instance4 = request_container.resolve_provider(request_singleton)
...
def test_app_singleton_in_request_scope() -> None:
    with SyncContainer() as app_container:
        with app_container.build_child_container() as request_container:
            singleton1 = request_container.resolve_provider(app_singleton)

        with app_container.build_child_container() as request_container:
            singleton2 = request_container.resolve_provider(app_singleton)
...
async def test_singleton_overridden() -> None:
    async with AsyncContainer() as app_container:
        ...
        singleton2 = await app_container.resolve_provider(app_singleton)
        singleton3 = await app_container.resolve_provider(app_singleton)

        app_container.reset_override(app_singleton)

        singleton4 = await app_container.resolve_provider(app_singleton)
...
def test_singleton_wrong_dependency_scope() -> None:
...
async def test_singleton_asyncio_concurrency() -> None:
    ...
    async def resolve_factory(container: AsyncContainer) -> SimpleCreator:
        return await container.resolve_provider(factory_with_resource)
...
def test_singleton_threading_concurrency() -> None:
    ...
    def resolve_singleton(container: SyncContainer) -> str:
...
def test_singleton_wth_resource_threading_concurrency() -> None:
    ...
    def resolve_factory(container: SyncContainer) -> SimpleCreator:
        return container.resolve_provider(factory_with_resource)
